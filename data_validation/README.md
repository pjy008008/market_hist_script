...
`quality_control.py`는 `daily_pipeline.py`에서 사용하는 자동 품질 검사 모듈입니다. 수집한 SIP 1분봉의 정규장 누락, 중복, OHLC 관계와 음수 거래량을 검사하고 최근 10거래일의 누락 구간만 Alpaca에 재요청합니다. 복구되지 않은 누락은 실제 무거래일 수 있으므로 원본을 보간하지 않고 `report/data_quality/`의 CSV 보고서에 남깁니다.
